...
addLoadEvent(function(){mermaid.initialize({startOnLoad:true,theme:'forest'})});
    </script>
    """
    ...
    self.body.append(tag_template.format(align=node.get('align'),
...
def render_mm_html(self, node, code, options, prefix='mermaid',
    ...
    try:
        if format not in ('png', 'svg'):
            raise MermaidError("mermaid_output_format must be one of 'raw', "
                               "'png', 'svg', but is %r" % format)
...
def render_mm_latex(self, node, code, options, prefix='mermaid'):
    ...
    if self.builder.config.mermaid_pdfcrop != '':
        ...
        try:
            ...
            if err.errno != ENOENT:   # No such file or directory
                raise
            self.builder.warn('command %r cannot be run (needed to crop pdf), '
