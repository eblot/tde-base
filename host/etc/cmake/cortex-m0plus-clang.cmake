#-----------------------------------------------------------------------------
# Definition file for Clang Cortex-M0+ toolchain
#-----------------------------------------------------------------------------
...
SET (xcc ${xclang})
# force GNU linker as LLVM is not able to generate ARMV6M valid veneers
# SET (XLD "arm-none-eabi-ld")
...
IF ( DEFINED XLD )
  FIND_PROGRAM (xld ${XLD})
  SET (XLDFLAGS "-nostdlib")
ELSE ()
  SET (LDCALL "-Wl,")
  SET (XLDFLAGS "-nostdlib -target ${XTOOLGENERIC}")
ENDIF ()
...
SET (CMAKE_C_LINK_FLAGS ${XLDFLAGS})
SET (CMAKE_CXX_LINK_FLAGS ${XLDFLAGS})
...
MACRO (use_target)
  ...
  LIST (APPEND PROJECT_LINK_LIBRARIES compiler_rt)
  STRING (REGEX REPLACE "plus$" "+" cpu ${TARGET})
  ...
  SET (XCC_MIN_OPTIMIZATION_LEVEL "s")
  SET (XCC_MAX_OPTIMIZATION_LEVEL "s")
...
MACRO (use_default_xxx_settings)
  ...
  SET (CXX_FEAT_DEFS "-fshow-column -fno-strict-aliasing")
  SET (CXX_FEAT_DEFS
        "${CXX_FEAT_DEFS} -fdiagnostics-color=always -fansi-escape-codes")
  SET (CXX_FEAT_DEFS
        "${CXX_FEAT_DEFS} -fdiagnostics-fixit-info")
  IF (XCC_VERBOSE)
  ...
  SET (CMAKE_CXX_FLAGS_RELEASE "-O${XCC_RELEASE_OPT} -DNDEBUG ${CXX_OPTS_RELEASE}")
  SET (EXTRA_DEFS_DISALLOWED 1)
ENDMACRO ()
...
MACRO (link_app app)
  ...
  WHILE ("LINK_SYMBOL" IN_LIST arguments)
    ...
    LIST (APPEND symbols "${LDCALL}--defsym=${symboldef}")
  ENDWHILE ()
  IF (EXTRALIBS)
  ...
  IF (link_script)
    ...
    IF (link_script_dir)
      SET (link_script_opt "${LDCALL}-L${link_script_dir}")
    ELSE ()
      SET (link_script_opt "")
    ENDIF ()
  ENDIF ()
  IF (link_dir)
      SET (link_script_opt "${LDCALL}-L${link_dir} ${link_script_opt}")
  ENDIF ()
